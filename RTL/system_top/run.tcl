cd functional_verification
set total_test_cases [exec python test_cases_generator.py]

cd ..
exec cmd.exe /c "vlib work"
exec cmd.exe /c "vlog system_top.v > system_top.log"
exec cmd.exe /c "vlog functional_verification/system_top_tb.v > functional_verification/system_top_tb.log"
exec {*}[auto_execok start] vmap -c
exec {*}[auto_execok start] vsim -c work.system_top_tb -do "run -all; mem save -o functional_verification/register_file_verilog.mem -f binary -wordsperline 1 /system_top_tb/U_system_top/U_register_file/memory; quit -f"
